...
#include "concurrency.h" // We are borrowing from librealsense concurrency infrastructure for this sample
#include <algorithm>
...
{
public:
    bit_packer(int digits) : _digits(digits), _bits(digits, false)
    {
    }
    ...
    void detect() 
    {
        ...
        while (_alive)
        {
            ...
            if (_queue.try_dequeue(&r))
            {
                ...
                HoughCircles(color_mat(roi), circles, CV_HOUGH_GRADIENT, 1, 10, 100, 30, 1, 100);
                for (size_t i = 0; i < circles.size(); i++)
                {
                    Vec3i c = circles[i];
                    Rect r(c[0] - c[2] - 5, c[1] - c[2] - 5, 2 * c[2] + 10, 2 * c[2] + 10);
                    rectangle(color_mat, r, Scalar(0, 100, 100),2);
                }
